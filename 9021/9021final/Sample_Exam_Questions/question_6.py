...
def f(for_seed, n, upper_bound):
    ...
    for l in range(len(square)):
        for c in range(len(square[l])):
            if square[l][c] in visit_ls and square[l][c] not in c_ls:
                c_ls.append(square[l][c])
            visit_ls.append(square[l][c])
    visit_ls.sorted()
    
    
    if c_ls:
        print(f'It is not a good square because it contains duplicates, namely: {' '.join(c_ls)}')
    else:
        print(f'Ordering the elements from left to right column, from top to bottom, yields:')
        c, ls = 0, []
        for m in range(len(square)):
            for n in range(len(square[l])):
                square[m][n] = visit[c]
                c+=1
        for a in square:
            print(a)
