...
def f(for_seed, n, upper_bound):
    ...
    for r in range(len(square)):
        for c in range(len(square[r])):
            if square[r][c] in v_ls:
                good = False
                duplicates.add(square[r][c])
            v_ls.append(square[r][c])
    if good:
        ordered_square = [[] for i in range(len(square))]
        for k in range(len(sorted(v_ls))):
            ordered_square[k%len(square)].append(sorted(v_ls)[k])
