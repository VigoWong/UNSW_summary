...
def words(text):
    ...
    '''
    ls = [a.lower() for a in text.split()]
    ls = [l.strip('.').strip(',').strip('!').strip('?').strip(';').strip(':') for l in ls]
    o_dict = defaultdict(list)
    for l in ls:
        o_dict[len(l)].append(l)
    for i in sorted(o_dict.keys()):
        print(f'Words of length {i}:')
        lss = sorted(set(o_dict[i]))
        print(f'    {lss}')
