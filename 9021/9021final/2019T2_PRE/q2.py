...
def check_out_square_and_fix_if_corrupted(square):
    ...
    set_1 = set([i for i in range(1 , len(square)**2+1)])
    set_2 = set([i for row in square for i in row])
    set_3 = set_2 - set_1 
    if not set_3:
        good_square = True
    elif len(set_3) == 1 and 0 in set_3 and len(set_2) == len(square)**2:
        corrupted_square = True
        target = list(set_1 - set_2)[0]
