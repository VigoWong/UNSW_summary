...
def f(a, b):
    ...
    for i in range(a, b+1):
        for j in range(i,b+1):
            if (i + j)%2 == 0:
                av = (i+j)//2
                if sorted(list(str(i))) == list(str(i)) and \
                sorted(list(str(j)), reverse = True) == list(str(j)) and \
                len(list(str(i))) == len(set(str(i))) and \
                len(list(str(j))) == len(set(str(j))):
                    if sorted(list(str(av))) == list(str(av)) or \
                    sorted(list(str(av)),reverse = True) == list(str(av)):
                        if len(list(str(av))) == len(set(str(av))):
                            ls.append((i, j , av))
    for i in ls:
        print(f'{i[0]} and {i[1]} with {i[2]} as average')
